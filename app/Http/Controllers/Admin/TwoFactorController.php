...
/**
 * Admin-side equivalent of Portal\TwoFactorController — same underlying
 * mechanism (shared users.two_factor_* columns, same TwoFactorAuthenticator
 * service, same login-time challenge in AuthenticatedSessionController,
 * which already checks hasTwoFactorEnabled() regardless of role). Only the
 * settings UI itself was missing for admin accounts; nothing about the
 * actual TOTP/recovery-code logic differs.
 */
class TwoFactorController extends Controller
{
    ...
    public function disable(Request $request)
    {
        ...
        return redirect()->route('admin.two-factor.show')->with('status', 'Two-factor authentication has been disabled.');
    }
