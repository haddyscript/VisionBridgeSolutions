...
- **Config:** `config/services.php` → `gemini.key` / `gemini.model` / `gemini.daily_message_limit`, sourced from `GEMINI_API_KEY` / `GEMINI_MODEL` / `AI_ASSISTANT_DAILY_LIMIT` in `.env`.
