...
- **Cost/rate limiting — DECIDED: 40 messages/client/day.** Configurable via `AI_ASSISTANT_DAILY_LIMIT` in `.env` (default 40). Past the limit, the widget shows a message asking the client to try again tomorrow or contact support directly.
- **LLM/provider — DECIDED (revised 2026-07-09): Google Gemini, via raw HTTP to the `generateContent` REST endpoint.** Originally built against Anthropic Claude (`claude-haiku-4-5`); switched same-day to Gemini (`gemini-2.5-flash` by default) at the client's request while budget is tight — Gemini has a persistent free API tier, unlike Claude/OpenAI which only offer expiring trial credit. No official Google PHP SDK is bundled for this project, so the integration calls the REST endpoint directly via Laravel's `Http` facade rather than pulling in a third-party wrapper package. Configurable via `GEMINI_MODEL` without a code change if a paid tier or different model is used later.

---

## 9. Implementation (built 2026-07-09, switched to Gemini same day)

- **Config:** `config/services.php` → `gemini.key` / `gemini.model` / `gemini.fallback_models` / `gemini.daily_message_limit`, sourced from `GEMINI_API_KEY` / `GEMINI_MODEL` / `GEMINI_FALLBACK_MODELS` / `AI_ASSISTANT_DAILY_LIMIT` in `.env`.
- **Data:** `assistant_conversations` (one per client, `escalated_at` flag) and `assistant_messages` (`role` + `content`, persisted turn by turn) — mirrors the `client_notifications` persistence pattern referenced in the decision above. The stored `role` values are always `user`/`assistant` regardless of provider — Gemini's `user`/`model` role naming is translated only at the API-call boundary in `AssistantService::reply()`, so switching providers again later doesn't require a data migration.
- **`App\Services\AssistantService`** — the whole flow: loads/creates the client's conversation, enforces the daily limit, computes account facts (project status/progress, pending one-time payments, Care Plan status/renewal/past-due) the same way the portal pages already do, builds the system prompt from this knowledge-base file + those facts + response rules, `POST`s to `https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent` (API key as a query parameter, per Gemini's auth model), and detects an escalation request from the model's reply via an internal sentinel token (`[[ESCALATE_TO_HUMAN]]`, stripped before the client ever sees it) — that's what creates the `ContactMessage` handoff row with the full transcript attached.
- **Model fallback chain — `AssistantService::generateContent()`** tries `gemini.model` first (`gemini-2.5-flash` by default), then walks `gemini.fallback_models` in order (default `gemini-2.5-flash-lite`, `gemini-3-flash`), advancing **only on a 429 (rate limited) response**. Any other failure (bad key, malformed request, Gemini server error) stops immediately and surfaces a friendly error rather than silently retrying through the whole chain. If every model in the chain is rate limited, the client sees a "getting a lot of traffic" message instead of a raw error. `gemini-2.5-flash-tts` (text-to-speech) and `gemini-robotics-er-1.5-preview` (robotics/embodied-reasoning) were deliberately left out of the default chain — they're not general-purpose chat models and `generateContent` likely won't return usable text from them — but they can be added via `GEMINI_FALLBACK_MODELS` if desired.
- **`App\Http\Controllers\Portal\AssistantController`** — `GET /portal/assistant` (conversation history + escalation state + remaining daily messages, for the widget's initial load) and `POST /portal/assistant` (send a message, get the reply).
- **UI:** `resources/views/portal/partials/assistant-widget.blade.php` — the floating gold bubble + slide-up panel, included from `layouts/portal.blade.php` so it's on every authenticated portal page. Vanilla JS, no build step, matches the WhatsApp-style bubble conventions already used for Revisions/Website Content.
- **Privacy boundary in practice:** `AssistantService::accountFacts()` is the only place account data reaches the prompt — it returns plain text lines, never a raw Eloquent model or array dump, so there's no path for the LLM to see fields beyond what that method explicitly writes out.
- **Known tradeoff of the free Gemini tier:** unlike a paid API tier, free-tier Gemini requests may be used by Google to improve their models. Client conversations here only ever contain computed facts (never raw account records), which limits exposure, but this is still worth revisiting once there's budget for a paid tier or for switching back to Claude.
