...
| Progress Tracker | Three donut progress rings at the top of the Overview page giving an at-a-glance snapshot: Onboarding (always 100% here, since the dashboard is only reachable once onboarding is complete), Project Build (milestones completed, from the project's progress percent), and Payments (amount paid vs. total; shows 100% / "Nothing due" when there are no charges). Rings and the Project Progress bar animate (sweep + count-up) on load |
| Notifications Recap | A card on the Overview page listing the client's 5 most recent notifications (time, title, description) with the unread count — the same data as the header notification bell, surfaced on the page |
...
| Post-Launch Feedback Prompt | After launch, the Overview page shows a "Your project launched — how did we do?" banner linking to the satisfaction survey. It now also pops up as a modal once per login (set in `AuthenticatedSessionController::finishLogin`, shown once via a session flag by `DashboardController`) until the survey is submitted, after which neither the banner nor the modal appears. Won't stack on top of the pending-payment reminder modal — that one takes priority for the login |
