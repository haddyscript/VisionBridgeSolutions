...
## 15jj. Admin Onboarding Tab — Full 5-Step Read-Only Preview (2026-07-11)

The admin Project Details "Onboarding" tab only showed 3 of the client's 5 onboarding-wizard steps (Care Plan, Service Agreement, Questionnaire) — Website Type and the Agreement Summary confirmation weren't represented at all. Rebuilt as 5 numbered step cards matching the client-facing wizard order (`portal.partials.onboarding-progress`'s `step` values) and gated by the same logic as `EnsureOnboardingComplete`:

1. Business Information (Questionnaire) — unchanged content, now under a numbered header
2. Website Type — new; shows `$project->website_type` or "Not selected yet"
3. Website Care Plan — unchanged content, now under a numbered header
4. Agreement Summary — new; this step has no data of its own (it's a confirm-and-continue screen), so its "Completed" status is inferred from `$project->user->onboarding_step >= 10` (set by `ServiceAgreementController::confirmSummary()`) or the agreement already being signed
5. Sign Service Agreement — unchanged content (preview/download links), now under a numbered header

Each card shows a "Completed" or "Not reached" badge — purely informational, no admin action of any kind lives on this tab (by design, per the original ask). The tab's own "Pending" badge (next to the tab label) now also accounts for `website_type` being unset, which it previously ignored.
