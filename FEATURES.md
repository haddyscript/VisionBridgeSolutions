...
- **Toggling an existing admin** (not just at creation) — `Admin\TeamController::toggleSuperAdmin()`, `PATCH /admin/team/{user}/super-admin`, same `super-admin` middleware gate. Each admin row shows a "Grant Super Admin" / "Revoke Super Admin" button (including for yourself, as long as at least one other super admin remains — same safety check as `destroy()`), so promoting or demoting someone no longer requires a direct SQL update.
