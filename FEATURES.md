...
- **Fixed same-day**: `@once` only emits the driver `<script>` at the position of the *first* `@include('admin._dropdown', ...)` on the page, so any dropdown instance appearing later in the raw HTML (e.g. the "Assign to…" pickers inside the Unassigned loop, which come after the workload filter) hadn't been parsed into the DOM yet when the script ran, and silently never got wired up. Fixed by deferring the wiring to `DOMContentLoaded` instead of running inline.
