...
- Admin page at `/admin/announcements` — create (as a draft, not auto-activated) with audience checkboxes, edit an existing one (title, message, audiences via a collapsible inline form), activate/deactivate, delete; the list shows each announcement's audience badges. Activate/deactivate posts to `admin.announcements.toggle`; content edits post to `admin.announcements.update`. Editing an active announcement's audiences re-runs the shared-audience dedup.
