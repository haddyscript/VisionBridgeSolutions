...
- **Brand Colors got a color-picker swatch** — `brand_colors` stays a plain string column (clients often mix names and hex, e.g. "Navy #111D33"), so a native `<input type="color">` button just appends the picked hex to the text field rather than replacing it, with a live row of swatches below parsed from whatever hex codes currently appear in the text.
