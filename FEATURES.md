...
- Only offered when the subscription is tied to a real `MaintenancePlan` tier with a `stripe_price_id` (admin-created ad-hoc-amount subscriptions have no "next tier" to offer). The local `Subscription` row's `maintenance_plan_id`/`description`/`amount`/`interval` update immediately on a successful Stripe call — no need to wait on the webhook, which doesn't touch those fields.
