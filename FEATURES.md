...
Previously, clients who signed a PDF-based agreement only ever saw the **blank** uploaded template plus a separate one-page signature certificate — the "PROJECT INVESTMENT," "SELECTED WEBSITE CARE PLAN," and "SIGNATURES" sections in the PDF itself always stayed empty (FEATURES.md section 10's original design). Added `App\Services\AgreementPdfFiller`, using `setasign/fpdi` + `setasign/fpdf` (new Composer dependencies — added directly via SSH on the server since Composer isn't available in the local dev environment; `composer.json`/`composer.lock` synced back afterward), to stamp the client's actual data onto the real PDF at signing time:

- **Filled at signing**: `Portal\ServiceAgreementController::store()` now also generates a filled copy (`AgreementPdfFiller::fill()`) whenever the active template `isPdfBased()`, saved to `service_agreement_signatures.filled_pdf_path` (new nullable column, migration `2026_07_05_000005`).
- **Fields filled** (scope decision, 2026-07-02 — deliberately limited to data already captured at signing, nothing invented): Selected Care Plan name + Monthly Investment (from the project's `CarePlanAgreement->maintenancePlan`), and the Client signature block — Organization Name, Authorized Representative, Title, drawn signature image, and Date. **Not filled**: the "PROJECT INVESTMENT" block (project price/deposit/dates aren't captured anywhere in the system), VisionBridge's own signature block, and "Effective Date of Agreement" — none of these have a data source yet.
- **Coordinates are hardcoded, not dynamic** — extracted precisely via `pdftotext -bbox-layout` against the specific PDF the boss uploaded ("...OFFICIAL 6.30.pdf", pages 132-134), not guessed from a screenshot. This is inherently tied to that exact document's layout and page numbers; **if a revised Master Agreement PDF is uploaded with different pagination, `AgreementPdfFiller`'s page-number constants and y-coordinates will need to be re-extracted and updated**, or filled agreements will silently land on the wrong page/position.
- The filled PDF is used in three places: the Documents page link (`portal.agreement.filled` route, replacing the blank-template link when a filled copy exists), the signed-agreement email attachment (`ServiceAgreementSignedMail`, replacing the blank template attachment), and is downloadable by admins too (same route, admin-or-owner authorization check).
- **Untested end-to-end as of this entry** — needs a real signing walkthrough on the server (where Composer/PHP actually run) to confirm the overlay lands correctly and that the `ext-gd`/`ext-zlib` PHP extensions `setasign/fpdf` requires are enabled.
