...
<div class="px-3 mb-2 relative">
    <button type="button" id="getting-started-toggle" class="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700/60 transition-colors text-left">
        <span class="relative w-8 h-8 shrink-0">
            <svg class="w-8 h-8 -rotate-90" viewBox="0 0 36 36">
                <circle cx="18" cy="18" r="15.5" fill="none" stroke="currentColor" stroke-width="3" class="text-gray-200 dark:text-white/10"/>
            ...
            @if ($gsCompleted < $gsTotal)
                <span class="absolute -top-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-red-500 ring-2 ring-white dark:ring-gray-800"></span>
            @endif
        </span>
        <span class="min-w-0 flex-1">
            <span class="flex items-center gap-1.5">
                <span class="text-sm font-semibold text-navy dark:text-white truncate">Getting Started</span>
                @if ($gsCompleted < $gsTotal)
                    <span class="text-xs font-semibold px-2 py-0.5 rounded-full bg-red-500 text-white shrink-0">{{ $gsTotal - $gsCompleted }}</span>
                @endif
            </span>
            <span class="block text-xs text-gray-400 dark:text-gray-500">{{ $gsCompleted }} of {{ $gsTotal }} tasks completed</span>
        </span>
        <svg id="getting-started-chevron" class="w-4 h-4 text-gray-300 dark:text-gray-600 shrink-0 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
    </button>
