...
@section('content')
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4">
        ...
        <div id="password-strength-group">
            <label class="block text-base font-bold text-navy mb-1">Password</label>
            <div class="relative">
                <input type="password" name="password" required
                       oninput="updatePasswordStrength(this, 'password-strength-group'); this.nextElementSibling.classList.toggle('hidden', !this.value)"
                ...
                <button type="button" onclick="togglePasswordVisibility(this)"
                        class="hidden absolute inset-y-0 right-0 flex items-center px-3 text-gray-400 hover:text-gray-600">
            ...
            <div class="password-strength-meter hidden mt-2">
                <div class="flex gap-1 mb-1">
                    <span class="strength-bar h-1 flex-1 rounded-full bg-gray-200 transition-colors"></span>
                    <span class="strength-bar h-1 flex-1 rounded-full bg-gray-200 transition-colors"></span>
                    <span class="strength-bar h-1 flex-1 rounded-full bg-gray-200 transition-colors"></span>
                </div>
                <p class="strength-label text-xs font-semibold"></p>
            </div>
            <p class="text-xs text-gray-500 mt-1.5">At least 8 characters, with an uppercase letter, a lowercase letter, and a number.</p>
        </div>
