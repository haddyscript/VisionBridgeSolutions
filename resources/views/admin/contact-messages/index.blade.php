...
<form method="GET" class="flex items-center justify-end gap-2.5 mb-5">
    <label class="text-[0.7rem] font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Sort by</label>
    <div class="relative">
        <select name="sort" onchange="this.form.submit()"
                class="appearance-none rounded-lg border border-gray-300 dark:border-gray-600 bg-white dark:bg-navy shadow-sm pl-3 pr-9 py-2 text-xs font-semibold text-navy dark:text-white focus:outline-none focus:ring-2 focus:ring-gold focus:border-gold cursor-pointer hover:border-gold/50 transition-colors">
...
@if ($messages->isEmpty())
    <div class="bg-white dark:bg-navy rounded-xl border border-gray-200 dark:border-gray-700 p-10 text-center">
        <p class="text-sm text-gray-500 dark:text-gray-400">No messages from the "Get in Touch" form yet.</p>
    </div>
@else
            ...
            <div onclick="window.location='{{ route('admin.contact-messages.show', $message) }}'"
                ...
                <div class="flex flex-wrap items-start justify-between gap-3">
                    <div class="flex items-start gap-2.5">
                        ...
                        <div>
                            <div class="flex items-center gap-2 flex-wrap">
                                <a href="{{ route('admin.contact-messages.show', $message) }}" class="text-sm font-semibold text-navy dark:text-white hover:text-gold-dark {{ $message->isRead() ? '' : 'font-bold' }}">
                                    {{ $message->displayName() }}
                                </a>
                                @if (! $message->isRead())
                                    <span class="text-[0.65rem] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-gold/15 text-gold-dark">New</span>
                                @endif
                                @if ($message->label)
                                    <span class="text-[0.65rem] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full {{ $labelColors[$message->label] ?? 'bg-gray-100 dark:bg-gray-700 text-gray-500' }}">
                                        {{ \App\Models\ContactMessage::LABELS[$message->label] ?? $message->label }}
                                    </span>
                                @endif
                            </div>
                            <div class="flex flex-wrap items-center gap-x-3 gap-y-1 mt-0.5">
                                <a href="mailto:{{ $message->email }}" onclick="event.stopPropagation()" class="text-xs text-gold-dark hover:underline">{{ $message->email }}</a>
                                @if ($message->organization)
                                    <span class="text-xs text-gray-500 dark:text-gray-400">{{ $message->organization }}</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    <div class="text-right shrink-0">
                        @if ($message->service)
                            <span class="inline-block text-[0.65rem] font-semibold uppercase tracking-wide px-2.5 py-1 rounded-full bg-gold/15 text-gold-dark mb-1">
                                {{ $message->service }}
                            </span>
                        @endif
                        <p class="text-[0.7rem] text-gray-500 dark:text-gray-400">{{ $message->created_at->format('M j, Y \a\t g:ia') }}</p>
                    </div>
                </div>
                ...
                <div class="flex justify-end mt-2 pt-2 border-t border-gray-100 dark:border-gray-700/60">
                    <form method="POST" action="{{ route('admin.contact-messages.toggle-read', $message) }}" onclick="event.stopPropagation()">
                        ...
                        @method('PATCH')
                        @if ($message->isRead())
                            <button type="submit" class="text-[0.7rem] font-semibold text-gray-500 dark:text-gray-400 bg-white dark:bg-navy border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-500 px-3 py-1 rounded-full transition-colors">
                                Mark as Unread
                            </button>
                        @else
                            <button type="submit" class="inline-flex items-center gap-1.5 text-[0.7rem] font-semibold text-gray-500 dark:text-gray-400 hover:text-navy dark:hover:text-white bg-white dark:bg-navy border border-gray-200 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700 hover:border-gray-300 dark:hover:border-gray-500 px-3 py-1 rounded-full transition-colors">
