            ...
            <div class="bg-white dark:bg-navy rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-5">
                @if ($submission->mission_statement)
                    <div>
                        ...
                        <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line">{{ $submission->mission_statement }}</p>
                    </div>
                @endif

                @if ($submission->vision_statement)
                    <div>
                        <h3 class="font-semibold text-navy dark:text-white mb-1.5">Vision Statement</h3>
                        <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line">{{ $submission->vision_statement }}</p>
                    </div>
                @endif
            </div>
        @endif
        ...
        @if (! empty($submission->services) || $submission->website_requirements)
            <div class="bg-white dark:bg-navy rounded-xl border border-gray-200 dark:border-gray-700 p-6 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach ($submission->services as $service)
                                <span class="text-xs font-medium px-2.5 py-1 rounded-full bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300">{{ $service }}</span>
                            @endforeach
                        </div>
                    </div>
                @endif
                ...
                @if ($submission->website_requirements)
                    <div>
                        <h3 class="font-semibold text-navy dark:text-white mb-1.5">Website Requirements</h3>
                        <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line">{{ $submission->website_requirements }}</p>
                    </div>
                @endif
            </div>
        @endif
            ...
            <div class="bg-white dark:bg-navy rounded-xl border border-gray-200 dark:border-gray-700 p-6">
                ...
                @elseif ($cat === 'video')
                    <div class="space-y-2.5">
                        @foreach ($items as $item)
                            <a href="{{ $item->url() }}" target="_blank" class="flex items-center gap-3 min-w-0 group rounded-lg border border-gray-200 dark:border-gray-700 px-4 py-3 hover:border-gold/40 transition-colors">
                                <span class="w-10 h-10 rounded-lg bg-navy/5 border border-gray-200 dark:border-gray-700 flex items-center justify-center shrink-0">
                                    <svg class="w-4 h-4 text-navy dark:text-white/60" fill="currentColor" viewBox="0 0 20 20"><path d="M6 4l10 6-10 6V4z"/></svg>
                                </span>
                                <span class="min-w-0">
                ...
                @else
                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                        @foreach ($items as $item)
                            <a href="{{ $item->url() }}" target="_blank" class="group rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden hover:border-gold/40 transition-colors">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="password" name="password" required autocomplete="current-password"
                               class="w-full rounded-lg border {{ $errors->has('password') ? 'border-red-400' : 'border-gray-300 dark:border-gray-600' }} px-3 py-2 pr-10 text-sm focus:outline-none focus:ring-2 focus:ring-red-400 focus:border-red-400 dark:bg-gray-900 dark:text-white">
