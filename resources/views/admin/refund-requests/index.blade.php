...
@else
    <div class="space-y-4">
        @foreach ($refundRequests as $item)
            <details class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden" {{ $item->isPending() ? 'open' : '' }}>
                <summary class="flex items-center justify-between gap-4 cursor-pointer list-none px-6 py-4 [&::-webkit-details-marker]:hidden">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-400 dark:text-gray-500">{{ $item->payment->description }} &middot; {{ $item->payment->formattedAmount() }}</p>
                        </div>
                    </div>
                    <div class="flex items-center gap-3">
                ...
                <div class="border-t border-gray-200 dark:border-gray-700 p-6">
                    <p class="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500 mb-1.5">Client's Reason</p>
                    <p class="text-sm text-gray-700 dark:text-gray-300 whitespace-pre-line mb-4">{{ $item->reason }}</p>

                    @if ($item->admin_notes)
                        <p class="text-xs font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500 mb-1.5">Admin Notes</p>
