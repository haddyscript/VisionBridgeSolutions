            ...
            <div class="chat-bubble group flex items-end {{ $isClient ? '' : 'justify-end' }} gap-2.5 max-w-[75%] {{ $isClient ? '' : 'ml-auto' }}"
                ...
                <div class="chat-bubble-card relative rounded-2xl {{ $isClient ? 'rounded-tl-sm bg-gray-100 dark:bg-gray-700/60' : 'rounded-tr-sm bg-navy text-white' }} px-4 py-2.5">
                    ...
                    @if (! $chatMessage->isDeleted())
                        ...
                        {{-- Anchored to the SAME side as the react button above
                             (not the opposite side) — otherwise the gap between
                             the button you click and the picker that opens grows
                             with the message's width, since the button sits just
                             outside the bubble's edge while an opposite-side
                             anchor is measured from the bubble's own far edge. --}}
                        <div class="chat-reaction-picker hidden absolute z-20 {{ $isClient ? 'right-8' : 'left-8' }} -top-11 items-center gap-0.5 bg-white dark:bg-navy border border-gray-200 dark:border-gray-700 rounded-full shadow-lg px-1.5 py-1">
...
function buildAdminBubbleHtml(data) {
    ...
    html += '<button type="button" class="chat-bubble-react-btn absolute -top-2 ' + (isClient ? '-right-9' : '-left-9') + ' opacity-0 group-hover:opacity-100 focus:opacity-100 w-6 h-6 rounded-full bg-white dark:bg-gray-700 shadow border border-gray-200 dark:border-gray-600 flex items-center justify-center text-gray-400 hover:text-gold-dark transition-opacity" title="React" aria-label="React to this message">' +
        '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14.828 14.828a4 4 0 01-5.656 0M9 10h.01M15 10h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>' +
    '</button>';
    html += '<div class="chat-reaction-picker hidden absolute z-20 ' + (isClient ? 'right-8' : 'left-8') + ' -top-11 items-center gap-0.5 bg-white dark:bg-navy border border-gray-200 dark:border-gray-700 rounded-full shadow-lg px-1.5 py-1">';
