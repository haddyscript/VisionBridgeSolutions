...
@section('title', 'Two-Factor Authentication – Client Portal')
...
<div class="max-w-lg space-y-6">
    ...
    @if (session('status'))
        <div class="text-sm text-teal-dark bg-teal/10 border border-teal/30 rounded-lg px-4 py-3">
            {{ session('status') }}
        </div>
    @endif
    ...
    @if ($recoveryCodes)
        <div class="bg-white dark:bg-gray-800 rounded-xl border-2 border-gold/40 p-6">
            ...
            <div class="grid grid-cols-2 gap-2 font-mono text-sm bg-gray-50 dark:bg-gray-900 rounded-lg p-4 mb-4">
                @foreach ($recoveryCodes as $code)
                    <span class="text-navy dark:text-white">{{ $code }}</span>
                @endforeach
            </div>
            <a download="visionbridge-recovery-codes.txt"
    ...
    @if ($enabled)
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form method="POST" action="{{ route('portal.two-factor.recovery-codes') }}" class="mb-4 space-y-3">
                @csrf
                ...
                <button type="submit" class="text-sm font-semibold text-navy dark:text-white bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 px-4 py-2 rounded-lg transition-colors">
                    Generate New Codes
                </button>
            </form>

            <form method="POST" action="{{ route('portal.two-factor.disable') }}" class="pt-4 border-t border-gray-100 dark:border-gray-700 space-y-3">
                @csrf
                <label class="block text-sm font-semibold text-navy dark:text-white">Disable two-factor authentication</label>
                <input type="password" name="password" placeholder="Current password" required
                       class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-gold focus:border-gold dark:bg-gray-900 dark:text-white">
                <button type="submit" class="text-sm font-semibold text-red-500 bg-red-50 hover:bg-red-100 px-4 py-2 rounded-lg transition-colors">
                    Disable 2FA
                </button>
            </form>
        </div>
    @else
        <div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-6">
            ...
            <form method="POST" action="{{ route('portal.two-factor.confirm') }}" class="space-y-3">
                @csrf
                ...
                <input type="text" name="code" required autocomplete="one-time-code" inputmode="numeric" placeholder="123456"
                       class="w-full rounded-lg border border-gray-300 dark:border-gray-600 px-4 py-2.5 text-sm text-center tracking-widest focus:outline-none focus:ring-2 focus:ring-gold focus:border-gold dark:bg-gray-900 dark:text-white">
