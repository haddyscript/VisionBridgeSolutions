...
@section('title', 'Announcements – Client Portal')
...
<div class="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
    <div class="flex items-center gap-2 mb-4">
        ...
        <span class="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-navy/5 dark:bg-white/10 text-navy/60 dark:text-white/60">{{ $announcements->count() }}</span>
    </div>
                ...
                <div id="history-item-{{ $announcement->id }}" class="rounded-lg border {{ $acknowledged ? 'border-gray-200 dark:border-gray-700' : 'border-gold/40 bg-gold/5' }} px-4 py-3">
                        ...
                        <span class="min-w-0 flex-1">
                            <span class="flex items-center flex-wrap gap-2">
                                ...
                                <span id="history-badge-{{ $announcement->id }}"
                                      class="text-[11px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full {{ $acknowledged ? 'bg-gray-100 dark:bg-white/10 text-gray-400' : 'bg-gold/15 text-gold-dark' }}">
                                    {{ $acknowledged ? 'Acknowledged' : 'Unacknowledged' }}
                                </span>
                            </span>
                            <span class="block text-xs text-gray-400 mt-0.5">{{ $announcement->created_at->format('M j, Y') }}</span>
                        </span>
                    ...
                    <div id="history-body-{{ $announcement->id }}" class="hidden mt-3">
                        @if ($announcement->subtitle || $announcement->event_date || $announcement->event_time)
                            <div class="mb-3 pb-3 border-b border-gray-100 dark:border-white/10">
                                @if ($announcement->subtitle)
                                    <p class="text-sm text-navy/60 dark:text-white/60 whitespace-pre-line">{{ $announcement->subtitle }}</p>
                                @endif
    ...
    function acknowledgeAnnouncementHistoryItem(id, btn) {
        fetch(btn.dataset.url, {
            ...
            if (badge) {
                ...
                badge.className = 'text-[11px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-gray-100 dark:bg-white/10 text-gray-400';
            }
