...
@else
    ...
    <div id="chat-thread" data-project-id="{{ $project->id }}" data-message-base-url="{{ url('/portal/chat-messages') }}"
        ...
        @php
            $chatEmojiSet = ['😀', '😃', '😄', '😁', '😆', '😅', '🤣', '😂', '🙂', '🙃', '😉', '😊', '😇', '🥰', '😍', '🤩', '😘', '😗', '😙', '😚', '😋', '😛', '😜', '🤪', '😝', '🤑', '🤗', '🤭', '🤫', '🤔', '🤨', '😐', '😑', '😶', '🙄', '😏', '😣', '😥', '😮', '😯', '😪', '😫', '🥱', '😴', '😌', '😔', '😟', '😕', '🙁', '☹️', '😖', '😞', '😢', '😭', '😤', '😠', '😡', '🤬', '😳', '🥵', '🥶', '😱', '😨', '😰', '😲', '🤒', '🤕', '🤢', '🤮', '🥴', '😵', '🤯', '🤠', '🥳', '😎', '🤓', '🧐', '👍', '👎', '👊', '✊', '🤛', '🤜', '🤞', '✌️', '🤟', '🤘', '👌', '🤏', '👈', '👉', '👆', '👇', '☝️', '✋', '🤚', '🖐️', '👋', '🤙', '💪', '🙏', '🤝', '🙌', '👏', '🫶', '👀', '🧠', '👂', '👁️', '❤️', '🧡', '💛', '💚', '💙', '💜', '🖤', '🤍', '🤎', '💔', '💕', '💞', '💓', '💗', '💖', '💘', '💝', '💟', '🐶', '🐱', '🐭', '🐹', '🐰', '🦊', '🐻', '🐼', '🐨', '🐯', '🦁', '🐮', '🐷', '🐸', '🐵', '🙈', '🙉', '🙊', '🐔', '🐧', '🐦', '🐤', '🦆', '🦅', '🦉', '🐴', '🦄', '🐝', '🐛', '🦋', '🐢', '🐍', '🐙', '🐬', '🐳', '🐠', '🍏', '🍎', '🍊', '🍋', '🍌', '🍉', '🍇', '🍓', '🍒', '🍑', '🥝', '🍍', '🥭', '🍅', '🌽', '🥕', '🍞', '🧀', '🥚', '🍗', '🍔', '🍟', '🌭', '🍕', '🌮', '🌯', '🥪', '🍿', '🍩', '🍪', '🎂', '🍰', '🍫', '🍬', '🍭', '🍦', '🍨', '☕', '🍵', '🥤', '🧃', '🍺', '🍷', '⚽', '🏀', '🏈', '⚾', '🎾', '🏐', '🎱', '🏓', '🏸', '🥊', '🎮', '🎲', '🎯', '🎨', '🎧', '🎸', '🎤', '🎬', '📷', '🎉', '🎊', '🎁', '🏆', '🥇', '⭐', '🌟', '✨', '🔥', '💯', '💫', '🌈', '☀️', '⛅', '🌙', '⚡', '❄️', '☔', '🌊', '🍀', '🌸', '🌺', '🌻', '🌹', '🚀', '✈️', '🚗', '🚲', '🚢', '🏠', '🌍', '🎈', '✅', '❌', '⚠️', '📌', '📎', '📅', '⏰', '💬', '📞', '💻', '📱', '🔔', '🔒', '🔑', '💡', '📝', '🙋', '🤷', '🤦', '🕺', '💃', '🚶', '🏃'];
            $chatQuickReplies = [
        ...
        @endphp
        {{-- Composer — Messenger-style single-row layout per the boss's
             reference: tool icons as filled circles outside the pill on the
             left, the pill itself just holds the textarea + emoji (docked
             inside on the right), and Send is the filled circle outside on
             the right, in the reference's "reaction/avatar" slot. Colored
             gold (this app's own accent) rather than Facebook's blue — every
             other primary action across the whole app already uses this
             same filled-gold-circle language, so matching that here instead
             of introducing a one-off blue kept it consistent with the rest
             of the portal. Same IDs/behavior as before throughout — only the
             layout changed, not the JS wiring (auto-expand, popovers, mic,
             counter, submit). --}}
        <div id="chat-composer-wrap" class="shrink-0 px-4 sm:px-6 py-4 border-t border-gray-100 dark:border-gray-700/60 bg-white dark:bg-gray-800">
            <form id="chat-thread-form" data-mark-read-url="{{ route('portal.chat.read', $project) }}" data-typing-url="{{ route('portal.chat.typing', $project) }}" data-no-loading-overlay
                ...
                {{-- Left icon cluster (outside the pill), filled circles. --}}
                <div class="flex items-center gap-1 sm:gap-1.5 shrink-0 pb-1">
                    <button type="button" id="chat-mic-btn" title="Voice input" aria-label="Voice input" class="hidden relative w-10 h-10 sm:w-9 sm:h-9 rounded-full bg-gold/15 text-gold-dark hover:bg-gold/25 hover:scale-105 flex items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gold/30">
                    ...
                    <button type="button" id="chat-attachment-btn" title="Share a link" aria-label="Share a link" class="w-10 h-10 sm:w-9 sm:h-9 rounded-full bg-gold/15 text-gold-dark hover:bg-gold/25 hover:scale-105 flex items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gold/30">
                    ...
                    <button type="button" id="chat-templates-btn" title="Quick replies" aria-label="Quick reply templates" class="w-10 h-10 sm:w-9 sm:h-9 rounded-full bg-gold/15 text-gold-dark hover:bg-gold/25 hover:scale-105 flex items-center justify-center transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-gold/30">
                ...
                <div class="flex-1 min-w-0 flex items-end gap-1 rounded-[1.5rem] bg-gray-100 dark:bg-gray-900 pl-4 pr-1.5 py-1.5 transition-all duration-150 focus-within:ring-2 focus-within:ring-gold/40">
                    <textarea name="body" id="chat-composer-textarea" rows="1" maxlength="5000" placeholder="Message VisionBridge Team…" required
                              class="flex-1 min-w-0 resize-none bg-transparent py-1.5 text-sm text-navy-dark dark:text-white placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none transition-all duration-150"
                ...
                <button type="submit" id="chat-send-btn" title="Send" aria-label="Send message" class="shrink-0 w-10 h-10 sm:w-10 sm:h-10 rounded-full bg-gold hover:bg-gold-dark text-navy-dark flex items-center justify-center transition-all duration-200 shadow-sm hover:shadow-md hover:scale-105 focus:outline-none focus:ring-2 focus:ring-gold/40 focus:ring-offset-2 dark:focus:ring-offset-gray-800">
