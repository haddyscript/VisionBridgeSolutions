...
<head>
    <meta charset="UTF-8">
    <title>Receipt — {{ $subscriptionPayment->subscription->description }}</title>
        ...
        <div class="px-8 py-7 space-y-5">
            <div class="flex items-start justify-between pb-5 border-b border-gray-100">
                ...
                <div class="text-right">
                    ...
                    <p class="font-display text-2xl font-bold text-navy">{{ $subscriptionPayment->formattedAmountPaid() }} {{ strtoupper($subscriptionPayment->currency) }}</p>
                </div>
            </div>
            ...
            @if ($subscriptionPayment->subscription->current_period_end)
                <div class="flex items-center justify-between">
                    <span class="text-sm text-gray-500">Next Renewal</span>
                    <span class="text-sm font-semibold text-navy">{{ $subscriptionPayment->subscription->current_period_end->format('M j, Y') }}</span>
                </div>
            @endif

            <div class="flex items-center justify-between gap-4">
                <span class="text-sm text-gray-500 shrink-0">Invoice ID</span>
                <span class="text-sm font-mono text-navy truncate">{{ $subscriptionPayment->stripe_invoice_id }}</span>
            </div>

            @if ($subscriptionPayment->hosted_invoice_url)
                <div class="text-center">
                    <a href="{{ $subscriptionPayment->hosted_invoice_url }}" target="_blank" class="inline-flex items-center gap-1.5 text-sm font-semibold text-gold-dark hover:underline">
