    ...
    <div class="flex min-h-screen">
        ...
        <aside id="portal-sidebar" class="fixed inset-y-0 left-0 z-40 w-64 flex flex-col -translate-x-full md:translate-x-0 transition-transform duration-200 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700">
            ...
            <nav class="flex-1 overflow-y-auto gold-scrollbar py-5 px-3 space-y-1">
                ...
                {{-- Positioned 3rd in the sidebar (right after Overview /
                     Project Timeline) rather than further down with the
                     rest — always visually distinct from every other nav
                     item here too (not just on its own active route): a
                     permanent gold-tinted pill + a small "live" dot on the
                     icon, so the one genuinely real-time feature in this
                ...
                <a href="{{ route('portal.chat.show') }}"
                   ...
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all duration-200 {{ request()->routeIs('portal.chat.*') ? 'bg-gold text-navy-dark shadow-sm' : 'bg-gold/10 text-gold-dark dark:bg-gold/15 dark:text-gold hover:bg-gold/20 dark:hover:bg-gold/25' }}">
                    ...
                    @if (($unreadChatCount ?? 0) > 0)
                        <span id="portal-chat-nav-badge" class="text-xs font-semibold px-2 py-0.5 rounded-full {{ request()->routeIs('portal.chat.*') ? 'bg-navy-dark text-gold' : 'bg-gold text-navy-dark' }}">{{ $unreadChatCount }}</span>
                    @endif
                </a>
        ...
        <div id="portal-main" class="flex-1 md:ml-64 min-w-0 transition-[margin] duration-200">
            <header class="sticky top-0 z-20 bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 border-t-4 border-t-navy dark:border-t-gold h-[4.5rem] flex items-center px-4 sm:px-6 lg:px-8 gap-3 sm:gap-4 lg:gap-6">
                ...
                {{-- data-tooltip (not title) on these four — a shared JS-driven
                     ...
                     name either way. --}}
                {{-- Same speech-bubble glyph as the sidebar's own Chat link,
                     for a quick-access route to it that doesn't require the
                     sidebar to be open (e.g. collapsed rail, or mid-scroll on
                     a long page). Badge mirrors the sidebar's own unread
