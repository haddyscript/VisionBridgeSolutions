...
<head>
    ...
    <style>
        ...
        /* Header "Chat" icon — permanent gold tint + pulsing teal live-dot,
           ...
           blending into the plain theme-toggle pill beside it. */
...
<body class="font-sans antialiased text-gray-800 dark:text-gray-100 bg-gray-50 dark:bg-navy-dark min-h-screen">
    ...
    <div class="flex min-h-screen">
        ...
        <aside id="admin-sidebar" class="fixed inset-y-0 left-0 z-40 w-64 flex flex-col -translate-x-full md:translate-x-0 transition-transform duration-200" style="background:#111D33;">
            ...
            <nav id="admin-sidebar-nav" class="flex-1 overflow-y-auto gold-scrollbar py-5 px-3 space-y-0.5">
                ...
                @if (auth()->user()->canAccessAdminPage('chat'))
                    {{-- Positioned 3rd in the sidebar (right after All
                         Projects / My Work Orders) rather than further down
                         with the rest — always visually distinct from every
                         other nav item here too (not just on its own active
                         route): a permanent gold-tinted pill + a small
                         "live" dot on the icon, so the one genuinely
                         real-time feature in this sidebar doesn't read as
                         just another plain link an admin could easily
                         scroll past. --}}
        ...
        <div class="flex-1 md:ml-64 min-w-0">
            <header class="sticky top-0 z-20 bg-white dark:bg-navy border-b border-gray-200 dark:border-gray-700 h-16 flex items-center px-4 sm:px-6 lg:px-8 gap-4">
                ...
                @if (auth()->user()->canAccessAdminPage('chat'))
                    <a href="{{ route('admin.chat.index') }}" title="Chat" aria-label="Chat"
                       class="chat-nav-icon-btn relative w-9 h-9 rounded-lg flex items-center justify-center transition-all duration-300 ease-out focus:outline-none focus-visible:ring-2 focus-visible:ring-gold/60 focus-visible:ring-offset-2 dark:focus-visible:ring-offset-navy {{ request()->routeIs('admin.chat.*') ? 'bg-gold text-navy-dark shadow-sm hover:shadow-[0_4px_16px_rgba(201,168,76,0.55)] hover:scale-105' : 'bg-gold/10 text-gold-dark hover:bg-gold/20 hover:scale-110 hover:shadow-[0_4px_14px_rgba(201,168,76,0.4)]' }}">
                        <svg class="chat-nav-icon-svg w-5 h-5 transition-transform duration-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 12h.01M12 12h.01M16 12h.01M21 12c0 4.418-4.03 8-9 8a9.863 9.863 0 01-4.255-.949L3 20l1.395-3.72C3.512 15.042 3 13.574 3 12c0-4.418 4.03-8 9-8s9 3.582 9 8z"/>
                        </svg>
                        {{-- Pulsing teal "live" dot, same idea as the
                             sidebar's own — cues "this one's real-time" at a
                             glance. --}}
                        <span class="chat-nav-live-dot absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-teal {{ request()->routeIs('admin.chat.*') ? 'ring-2 ring-gold' : 'ring-2 ring-white dark:ring-navy' }}"></span>
                        @if ($unreadChatCount > 0)
                            <span id="admin-chat-header-badge" class="absolute -top-1.5 -right-1.5 min-w-[1.15rem] h-[1.15rem] px-1 rounded-full bg-red-500 text-white text-[0.65rem] font-bold leading-none flex items-center justify-center ring-2 ring-white dark:ring-navy">{{ $unreadChatCount > 9 ? '9+' : $unreadChatCount }}</span>
                        @endif
                    </a>
                @endif
            ...
            <main class="px-4 sm:px-6 lg:px-8 {{ request()->routeIs('admin.chat.*') ? 'py-3 sm:py-4' : 'py-8' }}">
                @if (session('status'))
